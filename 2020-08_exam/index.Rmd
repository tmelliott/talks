...
* @Berczi_2017: use of probabilistic arrival time information
...
```{r,echo=FALSE,fig.height=5,fig.width=9,out.width="100%",fig.align="center",cache=TRUE,warning=FALSE,message=FALSE}
load("nwfit.rda")
x <- lapply(names(all), function(x) assign(x, all[[x]], .GlobalEnv))
rm(x)
...
USE_SEGS <- c(1, 3)
nwp1 <- betas %>%
    filter(l %in% USE_SEGS) %>%
    mutate(l = paste("Segment", l)) %>%
    ggplot() +
    geom_point(aes(timestamp, length / travel_time),
        data = segdat %>% filter(l %in% USE_SEGS) %>% mutate(l = paste("Segment", l)),
        colour = "black", size = 0.5) +
    geom_path(aes(t, beta, group = .draw),
        colour = "red", alpha = 0.5) +
    facet_wrap(~l, ncol = 2, labeller = function(x) "") +
    theme_classic() +
    theme(
...
USE_SEGS <- paste("Segment", USE_SEGS)
nwp2 <- kf_fits %>%
    mutate(
...
(nwp1 + ggtitle("JAGS fitted to August 13")) /
 (nwp2 + ggtitle("KF fitted to August 20"))
```

<span style="background:#ff000030;padding:0.5em">
<span style="color:red">$\hat\beta_\ell$</span> $\pm 1.96\sqrt{\vphantom{x^2}P_\ell}$</span> and
<span style="background:#00000030;padding:0.5em">
<span style="color:red">$\hat\beta_\ell$</span> $\pm 1.96\sqrt{P_\ell + \phi_\ell^2}$
</span>
...
\[
    p(\alpha_j | \boldsymbol{\beta}_{k}, \boldsymbol{x}_{k}) \approx
...
- [GPS error distribution](#gps-error-distribution)
- [Information filter](#information-filter)
- [PF journey planning](#particle-filter-journey-planning)
...
<center>
![GPS error distribution](gps-error.jpg)
</center>
...
<center>
![Probability of capture/on-time-arrival](pf_results_1.png)
</center>

# Particle filter journey planning
<center>
<img src="pf_results_2.png" width="50%">
</center>
