...
\sum_{i=1}^N W_{k}^{(i)} \delta(A_m - A_{m}^{(i)})
\]
...
- what people expect
- many apps/displays only show a single number
- how to decide on the best single number?
- underestimate
    - frustration: ETA 5 min turns into 10 min wait
    - harder to plan journey
- overestimate
    - shorter wait than expected = happy
    - __miss bus entirely__ 
- loss function
    - should penalise bus arriving too early heavily, particularly for more far out predictions
...
- allows uncertainty to be expressed
    - at least $A_l$ mins away, could be up to $A_u$ mins
...
hist(A/60, 40, main = "", prob = T, xlab = "ETA (min)", xlim = range(A/60))
...
hist(A/60, 60, main = "", prob = T, xlab = "ETA (min)", xlim = range(A/60))
plot(names(eta), cumsum(eta / N), 
     type = "s", xlab="ETA (min)", ylab="P(A < ETA)",
     xlim = range(A/60), ylim = 0:1, yaxs = "i")
```
