...
- __Vehicle transition function__, $f$
...
\]
<div style='margin-bottom: 1em'></div>
- Predicts new state _before_ observing latest position
<div style='margin-bottom: 1em'></div>
\[
...
- __combines data from different routes__
...
\[ \mathbf{P}_{c-1|c-1} = \Sigma_{c-1|c-1}^{-1} \]
    - __information vector__ 
\[ \boldsymbol{p}_{c-1|c-1} = \Sigma_{c-1|c-1}^{-1}\hat\Theta_{c-1|c-1} \]
<div style="margin-bottom:0.5em"></div>
- __easy handling of multiple observations__
...
R_c^{j\ell} &= \min\{\mathrm{Var}(T_j^\ell | \boldsymbol{y}_{1:k}^\ell), R_\text{min}\}
\end{split}
\]
<div style="margin-bottom:0.5em"></div>
...
(\boldsymbol{p}_{c|c})_j &= (\boldsymbol{p}_{c|c-1})_j + \sum_\ell {i}_{c\ell}
\end{split}
\]
...
----
<h2>Network state prediction</h2>
