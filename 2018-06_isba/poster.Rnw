\documentclass[plainboxedsections,portrait,20pt]{sciposter}
...
<<vehicle_state_lh_,fig.width=2.5,fig.height=2.5,out.width='0.8\\linewidth',fig.align='center',fig.cap='$\\bY_k$ (red cross) is a bivariate normal r.v. with mean and variance represented by the black dot and concentric rings, respectively.',fig.scap=NA>>=
#fig.cap=c('Simulated particle trajectories from time $t_{k-1}$ to their predicted state $\\bX_k^{(i)}$ (left), and their computed location $h(\\bX_k^{(i)})$ (right). The reported bus location $\\bY_k$ is shown by a red cross.','$\\bY_k$ (red cross) is a bivariate normal r.v. with mean and variance represented by the black dot and concentric rings, respectively.','After resampling, a posterior sample of trajectories is obtained (orange).'),fig.scap=NA>>=
...
<<vehicle_state,fig.width=6,fig.height=3,out.width='0.4\\linewidth',fig.align='center',fig.show='hide'>>=
#fig.cap=c('Simulated particle trajectories from time $t_{k-1}$ to their predicted state $\\bX_k^{(i)}$ (left), and their computed location $h(\\bX_k^{(i)})$ (right). The reported bus location $\\bY_k$ is shown by a red cross.','$\\bY_k$ (red cross) is a bivariate normal r.v. with mean and variance represented by the black dot and concentric rings, respectively.','After resampling, a posterior sample of trajectories is obtained (orange).'),fig.scap=NA>>=
ggarrange(x2, m2, widths = c(3, 1))
...
@
    ...
    %     \begin{itemize}
        ...
        \begin{enumerate}
            \item predict trajectory of each particle 
                ...
                as shown in figure~\ref{fig:vehicle_state1}
                \begin{equation*}
    ...
    \begin{minipage}[t]{0.64\linewidth}
            \item assume $\bY_k$ is a noisy measurement of true 
                position with GPS error $\sigma^2_y$ (fig~\ref{fig:vehicle_state_lh}),
            ...
            \item update state by resampling particles with replacement, using likelihood weights (fig~\ref{fig:vehicle_state2})
                \begin{equation*}
                    w^{(i)} = \frac{p(\bY_k | \bX_k^{(i)})}{\sum_{j=1}^N p(\bY_k | \bX_k^{(j)})}
                \end{equation*}
        \end{minipage}
        \begin{minipage}[t]{0.34\linewidth}
<<vehicle_state_lh,fig.width=2.5,fig.height=2.5,out.width='0.8\\linewidth',fig.align='center',fig.cap='$\\bY_k$ (red cross) is a bivariate normal r.v. with mean and variance represented by the black dot and concentric rings, respectively.',fig.scap=NA>>=
map
@
        \end{minipage}
    ...
    % \begin{minipage}[t]{0.49\linewidth}
            ...
            \item use resulting trajectories to estimate vehicle speed along 
                ...
                \begin{itemize}
                    \item $v^j_k$ is the mean speed of the particles, and
                    \item $e^j_k$ is the variance of particle speeds for a vehicle
                \end{itemize}
        \end{enumerate}
...
\begin{figure}
    \centering
    \subfigure[Forecasted particle trajectories]{%
        \includegraphics{figure/vehicle_state-1}
        \label{fig:vehicle_state1}}
    \subfigure[Updated particle sample are resampling]{%
        \includegraphics{figure/vehicle_state-2}
        \label{fig:vehicle_state2}}
    \caption{The vehicle state consists of distance traveled (the $y$-axes in the graphs). 
        The predicted state for each particle involves a trajectory (a), and the final position
        can be mapped to a location on a map, which makes comparing the predictions to the observation (in red)
        intuitive and straightforward.}
    \label{fig:vehicle_state}
\end{figure}
...
\begin{minipage}[t]{0.49\linewidth}
    ...
    % \begin{minipage}[t]{0.59\linewidth}
        \begin{itemize}
            \item goal is to develop a model that can estimate
                \textbf{real-time} and \textbf{future} network state for use in
                arrival time prediction
            \item \textbf{network state} is defined as the average speed $\beta^j_r$
                of buses along road segment $j$ at time $t_r$
                % \begin{equation*}
                %     p(\beta^j_r | v^j_r) \propto p(\beta^j_r) p(v^j_r | \beta^j_r)
                % \end{equation*}
            \item historical data is used to determine the prior mean and variance
                (blue lines in figure~\ref{fig:road_state})
                \begin{equation*}
                    p(\beta^j_r) \sim N(\mu_j(t_r), \psi_j(t_r)^2)
                \end{equation*}
            \item parameters $\hat\beta^j_r$ and $P^j_r$ are the state estimate and variance, 
                respectively,
                estimated using an adapted \textbf{extended Kalman filter (EKF)} algorithm
                \begin{equation*}
                    p(\beta^j_r | v^j_r) \sim N(\hat\beta^j_r, P^j_r)
                \end{equation*}
            \begin{enumerate}
                \item predict future state, fig.~\ref{fig:road_state1}
                \begin{itemize}
                    \item \textbf{transition function}
                        and \textbf{system noise} defined such that 
                        $\hat\beta^j_r$ and $P^j_r$ \textbf{converge} to
                        $\mu_j(t_r)$ and $\psi_j(t_r)$, respectively
                    \item EKF predict equations used to recursively estimate state
                        in one~second intervals
                \end{itemize}

                \item update state, fig.~\ref{fig:road_state2}
                \begin{itemize}
                    \item use values from step~5 in section~\ref{sec:pf}
                    \item EKF update equations with observation $v^j_r$ and measurement
                    error $e^j_r$, respectively
                \end{itemize}
            \end{enumerate}

            \item \emph{Here, we are assuming a constant speed along the length of each road; 
                we are currently working on a model that allows variable speeds.}

            \item this transition function is also used in arrival time prediction
                (section~\ref{sec:pred})
                \begin{itemize}
                    \item takes time for vehicles to travel from current postiion to
                        road segments further down the route
                    \item ETAs need to account for predicted congestion behaviour
                        (for example before/after peak hour), which is defined 
                        in the prior
                \end{itemize}
    ...
    \begin{figure}
    \centering
<<road_state,fig.width=5,fig.height=3,out.width='0.4\\textwidth',results='asis',fig.show='hide'>>=
# ,fig.align="center",fig.cap=c('Predicted road state (black) converges to the prior (blue), in terms of both the mean and variance (solid and dashed lines, respectively).','Network state updated using observations of vehicle speed from the particle filter (in red, showing estimated measurement error).'),fig.scap=NA>>=
X <- x <- 20
P <- p <- 2
n <- 30
lambda <- 0
...
@
    ...
    \subfigure[Forecasted network state]{%
        \includegraphics{figure/road_state-1}
        \label{fig:road_state1}}
    \subfigure[Updated and re-forecasted state]{%
        \includegraphics{figure/road_state-2}
        \label{fig:road_state2}}
    \caption{Network road state predictions as defined by the transition function converge towards the prior mean
        and variance (in blue) over time. These state forecasts are used for predicting future travel times for arrival time prediction, and as the state prediction for the update step that occurs whenever a bus travels along the road segment (in red).}
    \label{fig:road_state}
\end{figure}
    ...
    \begin{minipage}[t]{0.59\linewidth}
        \begin{itemize}
            \item accurate arrival time predictions need to consider
                \textbf{current vehicle location}, \textbf{current traffic state},
                as well as \textbf{future traffic states}, traffic lights, and intermediate bus stop wait times
            \item for each particle, simulate journey along remainder of route
            \begin{itemize}
                \item simulate particle speed from network model (normal r.v.)
                    at time of arrival at each road segment in sequence
                \item introduce wait times (exponential) at intersections and bus stops
                \item calculate arrival times at each upcoming stop
            \end{itemize}
            \item yeilds a distribution of arrival times for each stop (fig~\ref{fig:predict_eta})
            \item ETAs are typically reported in discrete minutes.
                For example, the distribution in figure~\ref{fig:predict_eta} might be summarised with
            \begin{itemize}
            ...
            \item summary statistics need to be chosen such that,
                ...
                but also \textbf{minimising the probability that the bus arrives sooner than the ETA} 
        \end{itemize}
    \end{minipage}
    \hspace{0.01\linewidth}
    \begin{minipage}[t]{0.39\linewidth}
...
@
    ...
    \begin{itemize}
        ...
        \item current real-time C++ implementation takes up to 20~seconds on an 8-core Virtual Machine
            with 5000~particles per vehicle
    \end{itemize}
    \textbf{Next steps:}
    \begin{itemize}
        \item improve the network state model:
            \textbf{non-constant speeds} along a roads (i.e., $\mu_j(t, d)$ depends on time and distance along segment),
            include \textbf{covariates in state transition} (adjacent segments, yesterday's traffic, weather, etc.)
        \item develop a stop- and intersection-wait time model to more accurately simulate wait times 
