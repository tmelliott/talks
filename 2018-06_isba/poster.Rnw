...
\begin{minipage}[t]{0.59\linewidth}
    ...
    \begin{itemize}
        \item real time information (RTI) is a vital component of public transit systems,
            most importantly providing estimated times of arrival (ETAs) of vehicles
        \item transit vehicle tracking has been well studied in order to improve ETA accuracy
            \cite{Wall99analgorithm,Dailey_2001,Cathey_2003,Hans_2015},
            though these typically focus on individual vehicle or route modeling
        \item the major cause of unreliability, particularly in networks with poor infrastructure for 
            vehicles (e.g., priority lanes), is congestion
        \item it has been shown that using buses from multiple routes for prediction
            improves estimates \cite{Yu_2011},
            however this was not generalised to all roads throughout the network
        \item \textbf{we are proposing a generalised approach to modeling transit vehicles that
            simultaneously models traffic along roads independently of route,
            allowing for more accurate arrival time prediction by accounting for real-time traffic conditions}
...
\begin{minipage}[t]{0.33\linewidth}
    ...
    \begin{itemize}
        \item \textbf{GTFS} is an API specification for transit data \cite{GoogleDevelopers_2006}
        \item available in over 500 locations worldwide
        \item we construct a transit network (intersections and connecting road segments) by 
            from the raw GTFS data  
    \end{itemize}
...
\begin{minipage}[t]{0.65\linewidth}
    ...
    \begin{itemize}
        ...
        \item \textbf{particle filter}: general, flexible estimation method that 
            ...
            each transitioned independently
            so features such as multimodality (e.g., at bus stops) are easily handled
        % \item involves two steps: \emph{predict} future state, and \emph{update} state
        %     using likelihood function
        \item measurement function $h:\mathbb{R}\mapsto\mathbb{R}^2$ calculates particle's map position
            from distance traveled along shape
    \end{itemize}
