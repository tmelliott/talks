...
\begin{minipage}[t]{0.39\linewidth}
    ...
    \begin{minipage}[t]{0.59\linewidth}
        \begin{itemize}
            ...
            \item historical data is used to determine the prior mean and variance
                (blue lines in figures~\ref{fig:road_state1} and \ref{fig:road_state2})
                \begin{equation*}
    ...
    \hspace{0.01\linewidth}
    \begin{minipage}[t]{0.39\linewidth}
<<road_state,fig.width=4,fig.height=2.5,out.width='\\textwidth',fig.align="center",fig.cap=c('Predicted road state (black) converges to the prior (blue), in terms of both the mean and variance (solid and dashed lines, respectively).','Network state updated using observations of vehicle speed from the particle filter (in red, showing estimated measurement error).'),fig.scap=NA>>=
X <- x <- 20
P <- p <- 2
n <- 30
lambda <- 0
