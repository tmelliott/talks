...
\begin{itemize}
    ...
    \vspace{1em}
    \item \textbf{proposal}: 
        a generalised approach to modeling transit vehicles and network congestion
        to obtain reliable ETAs

    \item \textbf{test location}: Auckland, New Zealand, where Auckland Transport provides a publically 
        accessibly vehicle locations API 
