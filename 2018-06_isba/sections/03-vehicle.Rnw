...
\begin{itemize}
    ...
    \item \textbf{transition function} $f$ describes behaviour of a bus: 
        ...
        where $Q_{k-1}$ is system noise (in vehicle speed)
        \begin{equation*}
    ...
    \item \textbf{likelihood}: given $\hat\bX_k$, define distance between
        ...
        \begin{equation*}
            \left(\delta_k^2 / \sigma_y^2\right)
...
<<vehicle_state_pred,fig.width=6,fig.height=2.8,out.width='\\linewidth',fig.align='center'>>=
ggarrange(x2, m2, widths = c(3, 1))
@
<<vehicle_state_update,fig.width=6,fig.height=2.8,out.width='\\linewidth',fig.align='center'>>=
